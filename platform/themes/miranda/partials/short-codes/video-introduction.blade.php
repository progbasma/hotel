<section
    class="text-block with-bg pt-115 pb-115"
    @if ($background_image) style="background-image: url({{ RvMedia::getImageUrl($background_image) }});" @endif
>
    <div class="container">
        <div class="row align-items-center justify-content-center justify-content-lg-between">
            <div
                class="col-lg-5 col-md-8 col-sm-10 wow fadeInLeft"
                data-wow-delay=".3s"
            >
                <div class="block-text mb-small">
                    <div class="section-title mb-20">
                        <span class="title-tag">{!! BaseHelper::clean($title) !!}</span>
                        @if ($sub_title || $subtitle)
                            <h2>{!! BaseHelper::clean($sub_title ?: $subtitle) !!}</h2>
                        @endif
                    </div>
                    <p>{!! BaseHelper::clean($description ?: $content) !!}</p>
                    @if ($button_text)
                        <a
                            class="main-btn btn-filled mt-40"
                            href="{{ $button_url }}"
                        >{{ $button_text }}</a>
                    @endif
                </div>
            </div>
            <div
                class="col-lg-6 col-md-10 wow fadeInRight"
                data-wow-delay=".5s"
            >
                <div
                    class="video-wrap"
                    @if ($video_image) style="background-image: url({{ RvMedia::getImageUrl($video_image) }});" @endif
                >
                    @if ($video_url)
                        <a
                            class="popup-video"
                            href="{{ $video_url }}"
                        ><i class="fas fa-play"></i></a>
                    @endif
                </div>
            </div>
        </div>
    </div>
</section>
